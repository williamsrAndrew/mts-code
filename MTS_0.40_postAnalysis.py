...
class Motor(Frame):
	...
	def initUI(self):
		...
		# Make sliders
		self.slider1 = Scale(top_frame, from_=0, to_=255, orient = HORIZONTAL, label = "Startup Time")
		self.slider1.pack(side = LEFT, padx = 3, pady = 5)
		self.slider2 = Scale(top_frame, from_=0, to_=255, orient = HORIZONTAL, label = "Hold Time")
		self.slider2.pack(side = LEFT, padx = 3, pady = 5)
		self.slider3 = Scale(top_frame, from_=0, to_=255, orient = HORIZONTAL, label = "Cool-Down Time")
		self.slider3.pack(side = LEFT, padx = 3, pady = 5)
		self.slider4 = Scale(top_frame, from_=0, to_=95, orient = HORIZONTAL, label = "Max Power %")
	...
	def runTest(self):
		...
		try:
			...
			if testNum == '1':
				ser.write(self.slider1.get())
				ser.write(self.slider2.get())
				ser.write(self.slider3.get())
				ser.write(self.slider4.get())
			elif testNum == '2':
				ser.write(self.slider1.get())
				ser.write(self.slider2.get())
				ser.write(self.slider3.get())
	...
	def pickChange(self, *args):
		...
		if option == '1':
			self.slider1.config(from_=0, to_=255, orient = HORIZONTAL, label = "Startup Time")
			self.slider1.pack(side = LEFT, padx = 3, pady = 5)
			self.slider2.config(from_=0, to_=255, orient = HORIZONTAL, label = "Hold Time")
			self.slider2.pack(side = LEFT, padx = 3, pady = 5)
			self.slider3.config(from_=0, to_=255, orient = HORIZONTAL, label = "Cool-Down Time")
			self.slider3.pack(side = LEFT, padx = 3, pady = 5)
