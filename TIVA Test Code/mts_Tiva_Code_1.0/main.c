...
uint16_t ReadI2CADC(uint8_t selectByte){
	...
	ROM_SysCtlDelay(SysCtlClockGet()/2000);
...
void GatherData(){
	...
	timeMilli = (TimerLoadGet64(WTIMER0_BASE) - TimerValueGet64(WTIMER0_BASE));
	timeMilli = timeMilli*1000;
	timeMilli = timeMilli/ROM_SysCtlClockGet();
...
void TestProgram1(void){	// single-cycle test
	...
	ROM_SysCtlDelay(SysCtlClockGet()/500);
